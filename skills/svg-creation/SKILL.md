...
Written for creative professionals. If tools aren't configured, explain setup clearly. Do not assume the user has technical knowledge.
...
If the user asked for transparent background, or it makes sense for the image, remove the background from the raster:
...
- This step is not needed if using layer decomposition, as usually the background is extracted to a separate layer, and can just be excluded from the SVG.

### Step 3 (Optional): Layer Decomposition
...
Then trace each layer individually and composite in SVG. This step requires fal.ai API key.
...
- **Presets**: `default`, `sharp`, `curvy`, `detailed`, `smoothed`, `posterized1`, `randomsampling2`, `grayscale`
- Recommended: `sharp` for illustrations with clean edges, `curvy` for more organic shapes, `posterized1` for two-color images.
- JSON options can be passed as a string instead of a preset. See docs for full options: https://raw.githubusercontent.com/mringler/image-tracer-ts/refs/heads/main/README.md
